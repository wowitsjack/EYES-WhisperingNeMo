...
# Language mappings and utilities
punct_model_langs = [
...
TO_LANGUAGE_CODE = {
    **{language: code for code, language in LANGUAGES.items()},
    ...
    "moldovan": "ro", "sinhalese": "si", "castilian": "es",
}
...
# Word and speaker mapping utilities
def get_word_ts_anchor(s, e, option="start"):
    """Get anchor timestamp for a word."""
    if option == "end":
        return e
    elif option == "mid":
        return (s + e) / 2
...
def get_first_word_idx_of_sentence(word_idx, word_list, speaker_list, max_words):
    """Find the first word index of a sentence."""
...
def get_last_word_idx_of_sentence(word_idx, word_list, max_words):
    """Find the last word index of a sentence."""
    is_word_sentence_end = lambda x: x >= 0 and word_list[x][-1] in sentence_ending_punctuations
    right_idx = word_idx
    while (
...
def get_realigned_ws_mapping_with_punctuation(
    ...
    """Realign word-speaker mapping using punctuation."""
...
def find_numeral_symbol_tokens(tokenizer):
    """Find tokens containing numerals or currency symbols."""
...
def _get_next_start_timestamp(word_timestamps, current_word_index, final_timestamp):
    """Get start timestamp for the next word."""
...
def filter_missing_timestamps(
    ...
    """Filter timestamps, handling missing values."""
...
def process_language_arg(language: str, model_name: str):
    """Process language argument to ensure validity."""
